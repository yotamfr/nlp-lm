...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    for j in xrange(BATCH_SIZE):
        n = np.random.randint(0,len(in_word_index))
        data[j] = num_to_word_embedding[in_word_index[n]]
        labels[j] = int_to_one_hot(out_word_index[n],output_dim)
...
def eval_neural_lm(eval_data_path):
    ...
    for i in xrange(num_of_examples):
        ...
        sum += np.log2(prob)
        if i % 10000 == 0:
            print "{0} iterations done so far. {1} to go".format(i,num_of_examples-i)

    l = sum[0]/float(num_of_examples)
